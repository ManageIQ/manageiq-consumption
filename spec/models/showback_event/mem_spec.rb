describe ManageIQ::Consumption::ShowbackEvent::MEM do
  ...
  context "memory in vm" do
    ...
    it "Calculate MEM_total_mem" do
      expect(event.MEM_sb_max_mem(1024)).to eq(4096)
      expect(event.MEM_sb_max_mem(4096)).to eq(4096)
    end
  end
  ...
  context "memory in container" do
    ...
    it "Calculate MEM_total_mem" do
      expect(event.MEM_sb_max_mem(1024)).to eq(1024)
      expect(event.MEM_sb_max_mem(0)).to eq(1024)
    end
  end
end
