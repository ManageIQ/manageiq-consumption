  ...
  context 'basic tests' do
    ...
    context 'rating with no context' do
      ...
      it 'calculates costs when rate is not found' do
        ...
        expect(event.data['Vm']['CPU']).not_to be_nil
        expect(event.data['Vm']['CPU']['average']).not_to be_nil
        expect(event.data['Vm']['CPU']['max_number_of_cpu']).not_to be_nil
    ...
    context 'rating with context' do
      ...
      it 'calculates costs when rate is not found' do
        ...
        event.reload
        # test that the event has the information we need in data
        expect(event.data['Vm']).not_to be_nil
        expect(event.data['Vm']['CPU']).not_to be_nil
        expect(event.data['Vm']['CPU']['average']).not_to be_nil
        expect(event.data['Vm']['CPU']['max_number_of_cpu']).not_to be_nil
        # Make rate category not found
      ...
      it 'calculates costs with one rate' do
        ...
        rate.save
        # Rating now should return the value
        expect(plan.calculate_total_cost(event)).to eq(fixed_rate + event.data['Vm']['CPU']['max_number_of_cpu'] * variable_rate )
      end
      ...
      it 'calculates costs when more than one rate applies' do
        ...
        rate2.save
        # Rating now should return the value
        expect(plan.calculate_total_cost(event)).to eq(rate.fixed_rate + rate2.fixed_rate + event.data['Vm']['CPU']['average'] * rate.variable_rate + event.data['Vm']['CPU']['max_number_of_cpu'] * rate2.variable_rate)
      end
