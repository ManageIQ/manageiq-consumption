class ManageIQ::Consumption::ShowbackRate < ApplicationRecord
  ...
  def occurrence(_value, event)
    ...
    [fixed_rate_subunits, variable_rate * event.time_span / event.month_duration]
  end
  ...
  def duration(value, event)
    ...
    [fixed_rate_subunits * event.time_span / event.month_duration, value * variable_rate * event.time_span / event.month_duration]
  end

  def quantity(value, event)
    # event.fixed_cost + variable_cost * event
    # [event.fixed_cost, event_measure * variable_cost]
    [fixed_rate_subunits * event.time_span / event.month_duration, value * variable_rate]
  end
end